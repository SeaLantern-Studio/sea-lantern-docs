...
- 确认命令格式正确（控制台中不需要 `/` 前缀）
- 确认服务器已完全启动（控制台出现类似 `Done (1.234s)! For help, type "help"` 的日志）
...
- 确认服务器已启动且控制台出现类似 `Done (1.234s)! For help, type "help"` 的日志
- 确认客户端版本与服务端版本一致
...
- 使用 Sea Lantern 内置的 P2P 联机创建隧道并生成联机凭证，无需公网 IP
...
Sea Lantern 是开源软件，代码公开可审查。服务器配置和存档保存在本地；核心下载、更新、插件市场和 P2P 联机等功能会按需访问网络服务。
