...
1. 點擊主介面的「建立伺服器」按鈕
2. 在引導介面中選擇匯入來源：伺服端 JAR、既有伺服器目錄或啟動腳本（`bat` / `sh`）
3. 如果還沒有伺服端，請參考 [伺服端核心取得](/zh-tw/server-jar)

### 啟動方式偵測與自訂命令

- Sea Lantern 會依匯入內容智慧偵測啟動方式
- 若偵測結果不符合預期，可手動切換並填寫自訂開服命令
- 建立流程支援整合包自動安裝，降低首次開服手動操作

### 選擇 Java 版本

完成匯入後，Sea Lantern 會列出系統中已偵測到的 Java 版本。選擇與您的 Minecraft 版本相符的 Java：
