...
- 確認命令格式正確（控制台中不需要 `/` 前綴）
- 確認伺服器已完全啟動（控制台出現類似 `Done (1.234s)! For help, type "help"` 的日誌）
...
- 確認伺服器已啟動且控制台出現類似 `Done (1.234s)! For help, type "help"` 的日誌
- 確認用戶端版本與伺服端版本一致
...
- 使用 Sea Lantern 內建的 P2P 連線建立隧道並產生連線憑證，無需公網 IP
...
Sea Lantern 是開源軟體，程式碼公開可供審查。伺服器設定和存檔儲存在本機；核心下載、更新、插件市場和 P2P 連線等功能會按需存取網路服務。
